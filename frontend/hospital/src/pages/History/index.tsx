...
const LogHistory = observer(() => {
  ...
  useEffect(() => {
    (async () => {
      const filter: any = {};
      if (selectedStartDate.checked)
        filter["startDate"] = selectedStartDate.date
          .toISOString()
          .split("T")[0];
      if (selectedEndDate.checked)
        filter["endDate"] = selectedEndDate.date.toISOString().split("T")[0];
      const { data: dataDebits } = await axios.post(
        "/transaction/search/group-by-service",
        {
          type: "Debit",
          ...filter,
        }
      ...
      const { data: dataCredits } = await axios.post(
        "/transaction/search/group-by-service",
        {
          type: "Credit",
          ...filter,
